...
class PotentialField(Node):
    ...
    def controller(self):
        ...
        delta = self.normalize_angle(angle - self.theta)
    ...
    def scan_callback(self, msg):
        ...
        for i in range(num_readings):
            ...
            angle = angle_min + self.theta + angle_increment * i
            if dist < 3:
                repulsive_force = 20 * ((1/dist) - (1/2)) * (1/(dist**2)) * (distance/dist)
                if np.linalg.norm(repulsive_force) > 14:
                    repulsive_force = repulsive_force / np.linalg.norm(repulsive_force) * 14
                x_r -= repulsive_force * math.cos(angle)
                y_r -= repulsive_force * math.sin(angle)
                self.V_repulsion.append([x_r,y_r])
                valid_points += 1
        '''for i in range(num_readings):
            distance = scan[i]
            if MIN_DISTANCE < distance < MAX_DISTANCE:
                repulsion_force = Q_repulsion / (4* math.pi * distance**2)
                angle = angle_min + self.theta + angle_increment * i
                x_r -= repulsion_force * math.cos(angle)
                y_r -= repulsion_force * math.sin(angle)
                self.V_repulsion.append([x_r,y_r])
                valid_points += 1'''

        if valid_points == 0:
            self.V_repulsion = [0.0001, 0.0000001]
        else:
            self.V_repulsion = [x_r, y_r]

        repulsion = self.publish_vector(self.V_repulsion[0], self.V_repulsion[1])
        self.rep_pub.publish(repulsion)
